// ==UserScript==
// @name          douban_movie_Lib
// @namespace     https://www.douban.com/people/57965364/douban_movie
// @description   douban_movie library
// @include       *
// @match         *
// @version       0.0.1
// @grant        GM_download
...
var save_As;
var html_Docx;
function downloadMovie(userId, db_c, db_w, db_d, save_as, html_docx) {
    uid = userId;
    db_collect = db_c;
    db_wish = db_w;
    db_do = db_d;
    save_As = save_as;
    html_Docx = html_docx;
    tempAlert("开始电影备份", 2000);
    collectionsUrl = "https://movie.douban.com/people/" + uid + "/collect";
    getPageMovie(collectionsUrl, db_collect, "豆瓣电影（看过）");
}

// 采集当前页数据，保存到indexedDB
function getPageMovie(link, db, filename) {
    fetch(link).then(res => res.text())
        .then(text => new DOMParser().parseFromString(text, 'text/html'))
        .then(function (data) {
            var items = getCurrentMoviePageList(data);
            db.items.bulkAdd(items).then(function () {
                setTimeout(function () {
                    ...
                    if (data.getElementsByClassName("next").length != 0 &&
                        ...
                        getPageMovie(nextUrl, db, filename);
                    } else {
                        exportAllMovie(db, filename)
                    }
                }, 1000);
...
// 获取当前页数据
function getCurrentMoviePageList(pagedocument) {
    var items = [];
    var movieList = pagedocument.getElementsByClassName("item");
    [].forEach.call(movieList, function (movie) {
        var titleElement = movie.getElementsByClassName("info")[0].getElementsByTagName("ul")[0].getElementsByClassName("title")[0].getElementsByTagName("a")[0];
        var title = titleElement.innerText.trim() || titleElement.textContent.trim();
        var intro = movie.getElementsByClassName("info")[0].getElementsByTagName("ul")[0].getElementsByClassName("intro")[0].innerHTML.trim();
        var rating;
        if (movie.getElementsByClassName("info")[0].getElementsByTagName("ul")[0]
            .getElementsByTagName("li")[2].getElementsByTagName("span").length > 1) {
            rating = movie.getElementsByClassName("info")[0].getElementsByTagName("ul")[0]
                .getElementsByTagName('li')[2].getElementsByTagName("span")[0].getAttribute("class").slice(6, 7);
        } else {
            rating = '';
        }
        var date = movie.getElementsByClassName("info")[0].getElementsByTagName("ul")[0]
            .getElementsByTagName('li')[2].getElementsByClassName("date")[0].innerHTML.trim();
        var tags;
        if (movie.getElementsByClassName("info")[0].getElementsByTagName("ul")[0]
            .getElementsByTagName('li')[2]
            &&
            movie.getElementsByClassName("info")[0].getElementsByTagName("ul")[0]
                .getElementsByTagName('li')[2].getElementsByClassName("tags")[0]) {
            tags = movie.getElementsByClassName("info")[0].getElementsByTagName("ul")[0]
        ...
        if (movie.getElementsByClassName("info")[0].getElementsByTagName("ul")[0]
            ...
            comment = movie.getElementsByClassName("info")[0].getElementsByTagName("ul")[0]
                .getElementsByTagName('li')[3].getElementsByClassName("comment")[0].innerHTML.trim();
        } else {
            comment = "";
        }
        items.push({
...
function exportAllMovie(db, file_name) {
    db.items.orderBy('date').toArray().then(function (all) {
        ...
        count++;
        if (count == 1) {
            wishsUrl = "https://movie.douban.com/people/" + uid + "/wish";
            getPageMovie(wishsUrl, db_wish, "豆瓣电影（想看）");
        } else if (count == 2) {
            dosUrl = "https://movie.douban.com/people/" + uid + "/do";
            getPageMovie(dosUrl, db_do, "豆瓣电影（在看）");
        } else {
            tempAlert("电影全部备份成功，开始备份影评（如果有的话）");
            reviewLing = "https://movie.douban.com/people/" + uid + "/reviews";
            parseReviewsPage(reviewLing, "影评");
        }
    });
}
